...
var import_dist7, import_dist8, import_dist9, AddressZero;
...
var import_dist10, import_dist11, import_dist12, NegativeOne, Zero, One, Two, WeiPerEther, MaxUint256, MinInt256, MaxInt256;
...
var import_dist13, import_dist14, import_dist15, HashZero;
...
var import_dist16, import_dist17, import_dist18, EtherSymbol;
...
var import_dist25, import_dist26, import_dist27, logger2, UnicodeNormalizationForm, Utf8ErrorReason, Utf8ErrorFuncs;
var init_utf8 = __esm({
  "node_modules/@ethersproject/strings/lib.esm/utf8.js"() {
    "use strict";
...
var import_dist31, import_dist32, import_dist33, Table_A_1_ranges, Table_B_1_flags, Table_B_2_ranges, Table_B_2_lut_abs, Table_B_2_lut_rel, Table_B_2_complex, Table_C_ranges;
var init_idna = __esm({
  "node_modules/@ethersproject/strings/lib.esm/idna.js"() {
    "use strict";
...
var import_dist46, import_dist47, import_dist48, version3;
...
var import_dist55, import_dist56, import_dist57, r, VALID, IGNORED, MAPPED, EMOJI_ROOT, HYPHEN, UNDERSCORE;
...
var import_dist58, import_dist59, import_dist60, logger3, Zeros;
...
var import_dist61, import_dist62, import_dist63, messagePrefix;
...
function checkString(key) {
  return function(value) {
    if (typeof value !== "string") {
      logger4.throwArgumentError(`invalid domain value for ${JSON.stringify(key)}`, `domain.${key}`, value);
    }
    return value;
  };
}
function getBaseEncoder(type) {
  {
    ...
    if (match) {
      ...
      if (width % 8 !== 0 || width > 256 || match[2] && match[2] !== String(width)) {
        logger4.throwArgumentError("invalid numeric width", "type", type);
      }
      ...
      return function(value) {
        ...
        if (v.lt(boundsLower) || v.gt(boundsUpper)) {
          logger4.throwArgumentError(`value out-of-bounds for ${type}`, "value", value);
        }
  ...
  }
  {
    ...
    if (match) {
      ...
      if (width === 0 || width > 32 || match[1] !== String(width)) {
        logger4.throwArgumentError("invalid bytes width", "type", type);
      }
      return function(value) {
        const bytes = arrayify(value);
        if (bytes.length !== width) {
          logger4.throwArgumentError(`invalid length for ${type}`, "value", value);
        }
...
var import_dist64, import_dist65, import_dist66, __awaiter2, logger4, padding, NegativeOne2, Zero2, One2, MaxUint2562, hexTrue, hexFalse, domainFieldTypes, domainFieldNames, domainChecks, TypedDataEncoder;
  ...
  "node_modules/@ethersproject/hash/lib.esm/typed-data.js"() {
    ...
    domainChecks = {
      ...
      chainId: function(value) {
        ...
        return logger4.throwArgumentError(`invalid domain value for "chainId"`, "domain.chainId", value);
      },
      verifyingContract: function(value) {
        ...
        return logger4.throwArgumentError(`invalid domain value "verifyingContract"`, "domain.verifyingContract", value);
      },
      salt: function(value) {
        try {
          const bytes = arrayify(value);
        ...
        return logger4.throwArgumentError(`invalid domain value "salt"`, "domain.salt", value);
      }
    };
    TypedDataEncoder = class _TypedDataEncoder {
      constructor(types) {
        ...
        for (const name in types) {
          ...
          types[name].forEach((field) => {
            if (uniqueNames[field.name]) {
              logger4.throwArgumentError(`duplicate variable name ${JSON.stringify(field.name)} in ${JSON.stringify(name)}`, "types", types);
            }
            uniqueNames[field.name] = true;
            const baseType = field.type.match(/^([^\x5b]*)(\x5b|$)/)[1];
            if (baseType === name) {
              logger4.throwArgumentError(`circular type reference to ${JSON.stringify(baseType)}`, "types", types);
            }
            ...
            if (!parents[baseType]) {
              logger4.throwArgumentError(`unknown type ${JSON.stringify(baseType)}`, "types", types);
            }
        ...
        if (primaryTypes.length === 0) {
          logger4.throwArgumentError("missing primary type", "types", types);
        } else if (primaryTypes.length > 1) {
          logger4.throwArgumentError(`ambiguous primary types or unused types: ${primaryTypes.map((t) => JSON.stringify(t)).join(", ")}`, "types", types);
        }
        defineReadOnly(this, "primaryType", primaryTypes[0]);
        function checkCircular(type, found) {
          if (found[type]) {
            logger4.throwArgumentError(`circular type reference to ${JSON.stringify(type)}`, "types", types);
          }
          found[type] = true;
      ...
      _getEncoder(type) {
        ...
        if (match) {
          ...
          return (value) => {
            if (length >= 0 && value.length !== length) {
              logger4.throwArgumentError("array length mismatch; expected length ${ arrayLength }", "value", value);
            }
        ...
        return logger4.throwArgumentError(`unknown type: ${type}`, "type", type);
      }
      encodeType(name) {
        const result = this._types[name];
        if (!result) {
          logger4.throwArgumentError(`unknown type: ${JSON.stringify(name)}`, "name", name);
        }
        return result;
      }
      ...
      static hashDomain(domain) {
        ...
        for (const name in domain) {
          ...
          if (!type) {
            logger4.throwArgumentError(`invalid typed-data domain key: ${JSON.stringify(name)}`, "domain", domain);
          }
      ...
      static getPayload(domain, types, value) {
        ...
        if (typesWithDomain.EIP712Domain) {
          logger4.throwArgumentError("types must not contain EIP712Domain type", "types.EIP712Domain", types);
        } else {
            ...
            switch (type) {
              ...
              case "string":
                if (typeof value2 !== "string") {
                  logger4.throwArgumentError(`invalid string`, "value", value2);
                }
                return value2;
            }
            return logger4.throwArgumentError("unsupported type", "type", type);
          })
        };
      }
    };
...
var import_dist70, import_dist71, import_dist72, SupportedAlgorithm;
...
var import_dist73, import_dist74, import_dist75, version4;
...
function computeHmac(algorithm, key, data) {
  if (!SupportedAlgorithm[algorithm]) {
    logger5.throwError("unsupported algorithm " + algorithm, Logger.errors.UNSUPPORTED_OPERATION, {
      operation: "hmac",
      algorithm
    });
  }
...
var import_dist76, import_dist77, import_dist78, import_hash, logger5;
var init_sha2 = __esm({
  "node_modules/@ethersproject/sha2/lib.esm/sha2.js"() {
    "use strict";
...
//# sourceMappingURL=chunk-26QOIHDW.js.map
